...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* colors */
    .stat-projects {
      background: linear-gradient(135deg, #0b66ff, #053a9b);
    }
...
<body>
  ...
  <div class="container">
      ...
      <form method="post" enctype="multipart/form-data">
        ...
        <div style="display:flex;gap:10px;margin-top:15px">
          ...
          <button type="button" onclick="clearForm()" class="btn"
            style="background: var(--gray);color:#0b66ff;border:2px solid #0b66ff">Clear</button>
        </div>
      </form>
    </div>
    ...
    <div id="tab-projects" class="tab card" style="display:none">
        ...
        <table>
          ...
          <tbody>
            <?php while ($p = $projects_res->fetch_assoc()) { ?>
              <tr>
                <td><?php if (!empty($p['image']) && file_exists(__DIR__ . '/uploads/' . $p['image'])) { ?><img
                      class="thumb" src="uploads/<?php echo e($p['image']); ?>"><?php } else
                  echo '-'; ?></td>
                ...
                <td>
                  ...
                  <form method="post" class="inline delete-form">
                    <input type="hidden" name="csrf" value="<?php echo e($csrf); ?>">
                    <button type="button" class="btn danger delete-btn" data-type="Project"
      ...
      <?php } else
        echo '<p class="small">No projects yet.</p>'; ?>
    </div>
    ...
    <div id="tab-orders" class="tab card" style="display:none">
          ...
          <tbody>
            <?php while ($o = $orders_res->fetch_assoc()) { ?>
              <tr>
                ...
                <td><?php echo e($o['service']); ?><?php if (!empty($o['otherservice']))
                     echo ' / ' . e($o['otherservice']); ?>
                </td>
                ...
                <td>
                  <form class="inline update-status-form" data-id="<?php echo $o['id']; ?>">
                    <select onchange="syncStatus(<?php echo $o['id']; ?>, this.value)">
                      <option value="pending" <?php if ($o['status'] == 'pending')
                        echo 'selected'; ?>>Pending</option>
                      <option value="processing" <?php if ($o['status'] == 'processing')
                        echo 'selected'; ?>>Processing
                      </option>
                      <option value="completed" <?php if ($o['status'] == 'completed')
                  ...
                  <form method="post" class="inline delete-form">
                    <input type="hidden" name="csrf" value="<?php echo e($csrf); ?>">
                    <button type="button" class="btn danger delete-btn" data-type="Order"
                      data-id="<?php echo $o['id']; ?>">Delete</button>
                  </form>
                </td>
              </tr>
            <?php } ?>
      ...
      <?php } else
        echo '<p class="small">No orders yet.</p>'; ?>
    </div>
